...
<Aside type="tip" title="Este tutorial é sobre a Linha de Comando">
...
<Aside  title="Saiba!">
  É de extrema utilidade esses conceitos, que serão usados durante toda a _Documentação para Desenvolvedores_ do Ladesa.
</Aside>
...
<Tabs syncKey="operatingSystem">
  <TabItem label="Windows" icon="seti:windows">
    Por muito tempo, o _CMD_ — também conhecido como _Prompt de Comando do Windows_ —, esteve presente junto ao _Power Shell_ nos sistemas operacionais _Windows / NT_. Os dois projetos forneceram por muito tempo tanto um emulador de terminal quanto o interpretador de shell.
    
    Atualmente, projeto _Power Shell_ — que conta com muitos novos recursos, atualizações e otimizações —, caminha para desepenhar cada vez mais o papel apenas de shell — sendo esse o recomendado para a utilização em ambientes _Windows_. Assim, o trabalho de projetar a janela visual fica a cargo do _Windows Terminal_.
  </TabItem>
  <TabItem label="GNU/Linux" icon="linux">
  </TabItem>
  <TabItem label="macOS" icon="apple">
...
<Aside type="tip" title="Saiba!">
  Durante toda a _Documentação para Desenvolvedores_ do Ladesa, será dado a preferência a especificação léxica do `GNU bash`.
</Aside>
